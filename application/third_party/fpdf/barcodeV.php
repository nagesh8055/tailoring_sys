...
$query="select vid from v2_vyavastha order by vid asc";
...
for($k=0;$k<$noOfPages;$k++)
{
	...
	for($i=0;$i<5;$i++)
	{
		...
		for($j=0;$j<4;$j++)
		{
			...
			$pdf->Image('images/icardManagementBack.jpg',($xaxis+($j*100)),$yaxis+($i*113),$cardWidth,$cardHeight,'JPG');
			//$pdf->Cell(($xaxis+($j*100)),$yaxis+($i*113),"Education");
					...
					try
					{
						...
						{
							$pdf->Image('images/data/data1/'.$id[$tmpIndex].'.png',(($xaxis+12)+($j*100)),($yaxis+52)+($i*113),65,30,'PNG');//img,x,y,h,w
						}
					}
					catch(Exception $e)
					{
